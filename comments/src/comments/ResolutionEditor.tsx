...
import { debounce, ActionButton } from 'dot11-components';


/* Inline styles */
//const styleMap: DraftStyleMap = {
const styleMap: { [styleName: string]: CSSObject } = {
	'BOLD': {
		'fontWeight': 'bold'
	},
	'ITALIC': {
...
function BlockStyleControls(props: {editorState: EditorState, onChange: (editorState: EditorState) => void}) {
...
function InlineStyleControls(props: {editorState: EditorState, onChange: (editorState: EditorState) => void}) {
	const {editorState, onChange} = props;
...
function ActionControls(props: {editorState: EditorState, onChange: (editorState: EditorState) => void}) {
	const {editorState, onChange} = props;
...
export class ResolutionEditor extends React.Component<ResolutionEditorProps, ResolutionEditorState> {
	...
	componentDidUpdate() {
		if (!this.state.editorState.getSelection().getHasFocus() && //hasFocus &&
	...
	initEditorState = (value: string | null) => {
	...
	onChange = (editorState: EditorState) => this.setState(state => ({...state, editorState}), this.debouncedSave);

	handleClear: React.MouseEventHandler = (event) => {
		event.preventDefault();	// don't take focus from editor
		const editorState = EditorState.push(this.state.editorState, ContentState.createFromText(''), 'remove-range' /*'remove-fragment'*/);
		this.onChange(editorState);
	}

	handleKeyCommand  = (command: string, state: EditorState): DraftHandleValue => {
