...
function usePrevBallotOptions(ballot_id: number) {
	...
	return React.useMemo(() => {
		...
		return ids
			.map(id => entities[id]!)
			.filter(b => b.groupId === ballot.groupId && b.Project === ballot.Project && new Date(b.Start || '') < new Date(ballot.Start || ''));
...
function SelectPrevBallot({
    ...
    const handleChange = (values: typeof options) => onChange(values.length? values[0].id: null)
	return (
		<Select
			values={values}
			options={options}
