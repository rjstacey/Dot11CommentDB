...
export const DELETE_COMMENTS_WITH_BALLOTID = 'DELETE_COMMENTS_WITH_BALLOTID'
export const DELETE_COMMENTS_WITH_BALLOTID_SUCCESS = 'DELETE_COMMENTS_WITH_BALLOTID_SUCCESS'
export const DELETE_COMMENTS_WITH_BALLOTID_FAILURE = 'DELETE_COMMENTS_WITH_BALLOTID_FAILURE'
...
export const UPLOAD_RESOLUTIONS = 'UPLOAD_RESOLUTIONS'
export const UPLOAD_RESOLUTIONS_SUCCESS = 'UPLOAD_RESOLUTIONS_SUCCESS'
export const UPLOAD_RESOLUTIONS_FAILURE = 'UPLOAD_RESOLUTIONS_FAILURE'
...
export function getComments(ballotId) {
	return async (dispatch) => {
		...
		try {
			...
			return Promise.all([
				dispatch(getCommentsFailure()),
				dispatch(setError('Unable to get comment list'))
			])
		}
	}
}

const deleteCommentsWithBallotIdLocal = (ballotId) => {return {type: DELETE_COMMENTS_WITH_BALLOTID, ballotId}}
const deleteCommentsWithBallotIdSuccess = (ballotId) => {return {type: DELETE_COMMENTS_WITH_BALLOTID_SUCCESS, ballotId}}
const deleteCommentsWithBallotIdFailure = (ballotId) => {return {type: DELETE_COMMENTS_WITH_BALLOTID_FAILURE, ballotId}}
...
const importCommentsSuccess = (ballotId, summary) => {return {type: IMPORT_COMMENTS_SUCCESS, ballotId, CommentsSummary: summary}}
...
export function updateComment(data) {
	return async (dispatch) => {
		...
		try {
			...
			return Promise.all([
				dispatch(updateCommentFailure()),
				dispatch(setError(`Unable to update comment ${data.BallotID}/${data.CommentID}`))
			])
		}
	}
}
...
export function addResolution(data) {
	return async (dispatch) => {
		...
		try {
			...
			await Promise.all([
				dispatch(addResolutionFailure()),
				dispatch(setError(`Unable to add resolution ${data.BallotID}/${data.CommentID}/${data.ResolutionID}`))
			])
			return -1
		}
	}
...
export function updateResolutions(ballotId, resolutions) {
	return async (dispatch) => {
		...
		try {
			...
			return Promise.all([
				dispatch(updateResolutionsFailure()),
				dispatch(setError(`Unable to update resolutions for ${ballotId}`))
			])
		}
	}
}
...
export function deleteResolution(data) {
	return async (dispatch) => {
		...
		try {
			...
			return Promise.all([
				dispatch(deleteResolutionFailure()),
				dispatch(setError(`Unable to delete resolution ${data.BallotID}/${data.CommentID}/${data.ResolutionID}`))
			])
		}
	}
}
...
export function uploadComments(ballotId, type, file) {
	return async (dispatch) => {
		...
		try {
			...
			return dispatch(uploadCommentsSuccess(ballotId, response.data.data.comments))
		}
		catch(error) {
			console.log(error)
			return Promise.all([
				dispatch(uploadCommentsFailure()),
				dispatch(setError(`Unable to upload comments for ballot ${ballotId}`))
			])
		}
	}
}

const uploadResolutionsLocal = (ballotId) => {return {type: UPLOAD_RESOLUTIONS, ballotId}}
const uploadResolutionsSuccess = (ballotId) => {return {type: UPLOAD_RESOLUTIONS_SUCCESS, ballotId}}
const uploadResolutionsFailure = () => {return {type: UPLOAD_RESOLUTIONS_FAILURE}}
...
export function uploadResolutions(ballotId, file) {
	return async (dispatch) => {
		...
		try {
			...
			if (response.data.status !== 'OK') {
				return Promise.all([
					dispatch(uploadResolutionsFailure()),
					dispatch(setError(response.data.message))
				])
			}
			return dispatch(uploadResolutionsSuccess(ballotId))
		}
		catch(error) {
			return Promise.all([
				dispatch(uploadResolutionsFailure()),
				dispatch(setError(`Unable to upload resolutions for ballot ${ballotId}`))
			])
		}
	}
}
