...
type MemberBallotParticipationInternalProps = MemberBallotParticipationProps & ConnectedMemberBallotParticipationProps;
...
class MemberBallotParticipation extends React.Component<MemberBallotParticipationInternalProps, MemberBallotParticipationState> {
	...
	generateColumns(props: MemberBallotParticipationInternalProps) {
		...
		return ballotSeriesParticipationColumns.map(col => {
			let renderCell: ((entry: BallotSeriesParticipationSummary) => JSX.Element | string | number) | undefined;
			if (col.key === 'project')
				renderCell = entry => ballotSeriesEntities[entry.id]?.project || '?';
			if (col.key === 'ballotIds')
				renderCell = entry => ballotSeriesEntities[entry.id]?.ballotIds.map(id => ballotEntities[id]?.BallotID || '?').join(', ') || '?';
			if (col.key === 'period')
