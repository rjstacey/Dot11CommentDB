...
function renderWorkingGroup({ item, props }: SelectItemRendererProps) {
	let label = item.name;
	if (!props.clearable) label += " Members";
...
export function PathWorkingGroupSelector(
	...
	async function handleChange(values: typeof options) {
		const group = await dispatch(
			setWorkingGroupId(values.length > 0 ? values[0].id : null)
		);
		if (group) dispatch(loadGroups(group.name));
		let pathName = location.pathname;
		if (groupName && group)
			pathName = pathName.replace(`/${groupName}`, `/${group.name}`);
		else pathName = group ? `/${group.name}` : "/";
		navigate(pathName);
	}
