...
import {
	loadImatMeetings,
	selectSyncedImatMeetingsEntities,
...
function imatMeetingToSession(meeting: ImatMeeting): Session {
...
type ColumnPropertiesWidth = ColumnProperties & { width: number };
...
function ImatMeetings() {
	...
	const [defaultSession, setDefaultSession] = React.useState<Session | null>(null);
	const numberSessions = React.useRef(20);

	const dispatch = useAppDispatch();
	const {valid} = useAppSelector(selectImatMeetingsState);
	const imatMeetings = useAppSelector(selectSyncedImatMeetingsEntities);
	const {valid: sessionsValid} = useAppSelector(selectSessionsState);

	const columns = React.useMemo(() => {

		const renderActions = ({rowData}: {rowData: any}) => 
			(rowData.sessionId)
	...
	const importSessionDialog = (meeting: ImatMeeting) => setDefaultSession(imatMeetingToSession(meeting));
