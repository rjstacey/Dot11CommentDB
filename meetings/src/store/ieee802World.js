...
export const importSelectedAsMeetings = () =>
	async (dispatch, getState) => {
		...
		for (const id of selected) {
			...
			meeting.organizationId = groupIds.find(id => groupEntities[id].name === subgroupName && orgIds.includes(groupEntities[id].parent_id));
			if (!meeting.organizationId) {
				meeting.organizationId = orgIds[0];
				if (!meeting.organizationId) {
			...
			meeting.start = DateTime.fromFormat(`${entry.breakoutDate} ${entry.startTime}`, 'yyyy-MM-dd HH:mm:ss', {zone: session.timezone});
			meeting.end = DateTime.fromFormat(`${entry.breakoutDate} ${entry.endTime}`, 'yyyy-MM-dd HH:mm:ss', {zone: session.timezone});
