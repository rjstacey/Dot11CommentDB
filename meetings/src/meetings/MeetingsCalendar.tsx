...
function MeetingContent({
	...
	const group = meeting.organizationId? groupEntities[meeting.organizationId]: undefined;
...
function RoomColumn({
	...
	const {timezone, timeslots, rooms} = useAppSelector(selectCurrentSession)!;
	...
	return (
		<Column key={room.id}>
			{timeslots.map((slot) => <TimeslotContent key={slot.id} date={date} room={room} slot={slot}/>)}
			{meetings.map((meeting) => <MeetingContent key={meeting.id} date={date} room={room} meeting={meeting} meetings={meetings}/>)}
		</Column>
	)
}

const SessionDayContainer = styled.div<{n: number}>`
	width: 100%;
	display: grid;
		...
		<Container n={shownDates.length} >
			...
			<div style={{gridArea: `1 / 2 / 1 / ${nDays + 2}`, position: 'sticky', top: 0, zIndex: '+2'}}>
				{day > 0 &&
					<ActionIcon
						type='angle-left'
						style={{position: 'absolute', top: 10, left: 10, fontSize: 'xx-large'}}
						onClick={clickLeft}
					/>}
				{(day + nDays) < dates.length &&
					<ActionIcon
						type='angle-right'
						style={{position: 'absolute', top: 10, right: 10, fontSize: 'xx-large'}}
						onClick={clickRight}
					/>}
			</div>
