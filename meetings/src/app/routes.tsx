...
import { useParams, useRouteError, Outlet, RouteObject, LoaderFunction } from 'react-router-dom';
...
const imatBreakoutsLoader: LoaderFunction = async ({params}) => {
	...
	if (groupName) {
		...
		if (access >= AccessLevel.ro) {
			...
			if (meetingNumber)
				dispatch(loadBreakouts(meetingNumber));
			else
...
/** A component that only renders its children of the user has a defined minimum access */
function GateComponent({minAccess, children}: {minAccess: number, children: React.ReactNode}) {
...
const groupRoutes = groupRoutes_ungated.map(r => {
	if (typeof r.minAccess === 'number')
		return {...r, element: <GateComponent minAccess={r.minAccess} children={r.element} />}
	return r;
})
