...
import { getGroupByName, getGroupRollUpPermissions, Group } from '../services/groups';
...
async function parseBallot_id(req: Request, res: Response, next: NextFunction) {
	...
	req.ballot = ballot;
	req.permissions = ballot.groupId? await getGroupRollUpPermissions(req.user, ballot.groupId): {};
	next();
}
