...
const legacyColumns = {
	...
	'Resolution': {
		width: 25,
		numFmt: '@',
		set: c => genResolution(c),
		get: (v, c) => parseResolution(v, c)
	},
	'Owning Ad-hoc': {
		width: 9,
		numFmt: '@',
...
function headingsMatch(headingRow, expectedHeadings) {
...
const getColRef = n => n? getColRef(Math.floor((n-1)/26)) + String.fromCharCode(65 + ((n-1) % 26)): '';

function addResolutionFormatting(sheet, columns, nRows) {
...
function addStatus(sheet, columns, nRows) {
...
function genWorksheet(sheet, columns, ballotId, doc, comments) {
