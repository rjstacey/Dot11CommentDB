...
export async function exportMembersPrivate(
	...
	type ParticipationRecord = Record<string, string>;
	...
	const { sessions, attendances } = await getRecentAttendances(user, groupId);
	const emptyAttendanceRecord: ParticipationRecord = {};
	for (const session of sessions) {
		const yearMonth = DateTime.fromISO(session.startDate).toFormat(
			"yyyy MMM"
		);
		emptyAttendanceRecord[session.type.toUpperCase() + ": " + yearMonth] =
			"";
	}

	const { ballotSeries, ballotSeriesParticipation } =
		await getActiveBallotSeriesParticipation(groupId);

	const emptyBallotSeriesRecord: ParticipationRecord = {};
	for (const ballotS of ballotSeries) {
		emptyBallotSeriesRecord[ballotS.project] = "N/A";
	}

	// Normalize members records
	const memberEntities: Record<
		number,
		Member & {
			attendanceRecord: ParticipationRecord;
			ballotSeriesRecord: ParticipationRecord;
		}
	> = {};
	const memberIds: number[] = [];
	for (const m of members) {
		memberEntities[m.SAPIN] = {
			...m,
			attendanceRecord: emptyAttendanceRecord,
			ballotSeriesRecord: emptyBallotSeriesRecord,
		};
		memberIds.push(m.SAPIN);
	}

	for (const attendance of attendances) {
		const member = memberEntities[attendance.SAPIN];
		if (!member) continue;

		// Find the date (if present) at which the member became a Non-Voter
		const nonVoterDateStr = member.StatusChangeHistory.find(
			(h) => h.NewStatus === "Non-Voter"
		)?.Date;
		const nonVoterDate = nonVoterDateStr
			? DateTime.fromISO(nonVoterDateStr)
			: null;
		...
		for (const session of sessions) {
			...
			let s = "";
			const a = attendance.sessionAttendanceSummaries.find(
				(a) => a.session_id === session_id
			);
			if (a) {
				if (
					(a.AttendancePercentage > 0.75 && !a.DidNotAttend) ||
					a.DidAttend
				) {
					if (
						nonVoterDate &&
						DateTime.fromISO(session.startDate) < nonVoterDate
					) {
						s = "—";
					} else {
						s = "✓";
			...
			member.attendanceRecord[
				session.type.toUpperCase() + ": " + yearMonth
			] = s;
		}
	}

	for (const participation of ballotSeriesParticipation) {
		const member = memberEntities[participation.SAPIN];
		if (!member) continue;
		...
		member.ballotSeriesRecord = {};
		for (const ballotS of ballotSeries) {
			let s = "N/A";
			const summary =
				participation.ballotSeriesParticipationSummaries.find(
					(a) => a.series_id === ballotS.id
				);
			if (summary) {
				if (summary.vote || summary.excused) {
					if (
						voterDate &&
						DateTime.fromISO(ballotS.start) < voterDate
					) {
						s = "—";
					} else {
						s = "✓";
			...
			member.ballotSeriesRecord[ballotS.project] = s;
		}
	}
	...
	let ssData = memberIds.map((id) => {
		...
		return {
			...
			Status: m.Status,
			...m.attendanceRecord,
			...m.ballotSeriesRecord,
		};
	});

	const csv = await csvStringify(ssData, { header: true, bom: true });
