 ...
 */
const getSessionAttendeesSQL = (session_id: number) => 
	'SELECT ' +
		'a.id, ' +
		'a.SAPIN, ' +
		'm.MemberID, ' +
...
const getBreakoutsSQL = (session_id: number) =>
	'SELECT ' +
		'*, ' +
...
const getBreakoutAttendeesSQL = (session_id: number, breakout_id: number) =>
	'SELECT ' +
		'a.SAPIN, ' +
...
const getAttendeeSessionCreditTable = (session_id: number) =>
...
const getSessionAttendanceSQL = (session_id: number) =>
	'SELECT ' +
		'a.SAPIN, ' +
...
export async function getBreakoutAttendees(user, session_id: number, breakout_id: number) {
	const sql =
...
export function getSessionAttendees(session_id: number) {
...
function attendanceSummaryEntry(a: Partial<AttendanceSummary>) {
	if (!a)
		return {};
...
export async function upsertMemberAttendanceSummaries(sapin: number, attendances: AttendanceSummary[]) {
	...
	let inserts: Partial<AttendanceSummary>[] = [],
		updates: Update<AttendanceSummary>[] = [];
