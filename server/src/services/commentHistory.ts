...
const createTriggerResolutionsUpdateSQL = `
	...
	BEGIN
		SET @action ="update";
		SET @changes = JSON_OBJECT();
		${resolutionEditableFields.map(f => `IF NOT(NEW.${f}<=>OLD.${f}) THEN SET @changes = JSON_INSERT(@changes, '$.${f}', NEW.${f}); END IF;`).join('\n\t\t')}
...
export async function getCommentHistory(comment_id: number) {
	...
	const results = await db.query({sql, dateStrings: true}) as [ CommentHistoryEvent[], Comment[], Resolution[] ];
	...
	commentsHistory
		.forEach(event => {
			...
			if (!event.resolution_id) {
				...
				history.push({...event, comment, resolution_id: undefined, resolution: undefined});
				comment = {...comment, ...event.Changes};
			}
			else {
				if (event.Action === "add") {
					...
					resolution = resolutionEntities[event.resolution_id] || defaultResolution;
				}
				resolutionEntities[event.resolution_id] = {...resolution, ...event.Changes};
				history.push({...event, comment, resolution_id: resolution.id, resolution});
			}
		});
