...
const matchPageLine = (dbValue: string, sValue: string) => {
	...
	if (Math.round(parseFloat(dbValue)) === parseInt(sValue)) // Legacy converts page to int
		return true;
	if (isNaN(Number(dbValue))) // Legacy ignores things that aren't numbers
		return true;
	return false;
}
...
type CommentComp = Pick<Comment, "Category" | "C_Clause" | "C_Page" | "C_Line" | "Comment" | "ProposedChange">
...
function findMatchByEliminationUsingTheseComparisons(dbC: CommentComp, sheetComments: CommentResolution[], comparisons: CompFunc[]) {
 ...
 */
function findMatchByElimination(dbC: Comment, sheetComments: CommentResolution[]) {
 ...
 */
const matchByElimination: MatchFunc = function(sheetComments: CommentResolution[], dbComments: CommentResolution[]) {
 ...
 */
const matchComment: MatchFunc = function(sheetComments: CommentResolution[], dbComments: CommentResolution[]) {
...
function commentUpdate(toUpdate: FieldToUpdate[], c: Partial<CommentUpdate>, cs: CommentUpdate & {CID: CommentResolution["CID"]}) {
...
async function updateComments(userId: number, ballot_id: number, matched: CommentMatch[], toUpdate: FieldToUpdate[]) {
...
type NewComment = Partial<Omit<Comment, "id" | "ballot_id" | "LastModifiedBy" | "LastModifiedTime">>;

async function addComments(userId: number, ballot_id: number, sheetComments: CommentResolution[], toUpdate: FieldToUpdate[]) {
